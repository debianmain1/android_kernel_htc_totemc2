...
static long kgsl_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	/* for KGSL out of memory debug log usage */
	else if (cmd == IOCTL_KGSL_DRAWCTXT_CREATE) {
		ctxt_count++;
		KGSL_CTXT_CRIT(dev_priv->device, "[DISP] KGSL creates context pid=%d, cur_ctxt_count=%d\n",
				(int)dev_priv->process_priv->pid, ctxt_count);
	}
	else if (cmd == IOCTL_KGSL_DRAWCTXT_DESTROY) {
		ctxt_count--;
		KGSL_CTXT_CRIT(dev_priv->device, "[DISP] KGSL destroy context pid=%d, cur_ctxt_count=%d\n",
				(int)dev_priv->process_priv->pid, ctxt_count);
	}
