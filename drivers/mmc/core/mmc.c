	...
	struct mmc_card *oldcard)
{
	...
	*/
	...
	if (card->ext_csd.enhanced_area_en) {
		...
		printk(KERN_INFO "%s: set ext_csd[175] = 1\n",
			mmc_hostname(card->host));
		err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
				 EXT_CSD_ERASE_GROUP_DEF, 1,
				 card->ext_csd.generic_cmd6_time);
		...
		/* Sandisk 24nm extreme 32G */
			...
			card->wr_perf = 12;
		/* Sandisk 19nm Ultra 8G */
		else if ((card->ext_csd.sectors == 15269888) && !strcmp(card->cid.prod_name, "ES308G"))
			card->wr_perf = 12;
	}
	...
	if (card && card->ext_csd.rev >= 3) {
		...
		*/
		if (card->ext_csd.part_config & EXT_CSD_PART_CONFIG_ACC_MASK) {
			card->ext_csd.part_config &= ~EXT_CSD_PART_CONFIG_ACC_MASK;
			err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PART_CONFIG,
				card->ext_csd.part_config, card->ext_csd.part_time);
			if (err && err != -EBADMSG)
				goto err;
		}
		...
		*/
		if ((card->csd.mmca_vsn >= CSD_SPEC_VER_4) &&
			...
			for (; idx < ARRAY_SIZE(bus_widths); idx++) {
				...
				ddr = 0; /* no DDR for 1-bit width */
				err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
					EXT_CSD_BUS_WIDTH, ext_csd_bits[idx][0], 0);
				if (!err) {
					mmc_set_bus_width(card->host, bus_width);
					break;
				}
			...
			if (!err && ddr) {
				err = mmc_switch(card, EXT_CSD_CMD_SET_NORMAL,
				EXT_CSD_BUS_WIDTH, ext_csd_bits[idx][1], 0);
			}
			if (err) {
				printk(KERN_WARNING "%s: switch to bus width %d ddr %d "
