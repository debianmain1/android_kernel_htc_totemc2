...
static bool is_ac_safety_timeout_enabled = false;
...
{
	u8 temp;
	int rc, idx_of_trim =0;
	 /* We keep original reg_val as index to be used in pm_chg_usb_trim()
	      because we will shift 1 bit later for setting 500/1100mA of iusbmax*/
	if (pm8xxx_get_version(chip->dev->parent) == PM8XXX_VERSION_8038)
		idx_of_trim = reg_val;

	/*Because usb_ma_table[] is changed, we need to shift 1 bit
	    for setting 500/1100mA of iusbmax */
	reg_val = usb_ma_table[reg_val].chg_iusb_value >> 1;
	...
	rc = pm_chg_masked_write(chip, PBL_ACCESS2, PM8921_CHG_IUSB_MASK, temp);
	if (rc)
		pr_err("Failed to write PBL_ACCESS2 rc=%d\n", rc);

	if (pm8xxx_get_version(chip->dev->parent) == PM8XXX_VERSION_8038)
	{
		rc = pm_chg_usb_trim(chip, idx_of_trim);
		if (rc)
...
static int pm_chg_iusbmax_get(struct pm8921_chg_chip *chip, int *mA)
{
	...
	*mA = 0;
	...
	/*Because usb_ma_table[] is changed, we need to shift 1 bit
	    for getting 500/1100mA of iusbmax */
	temp = temp << 1;
				...
				int usb_present)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (usb_present) {
			...
			is_ac_safety_timeout_enabled = false;
...
static irqreturn_t chgfail_irq_handler(int irq, void *data)
{
	...
	if (!is_ac_online() || flag_keep_charge_on || flag_pa_recharge) {
		...
		if(!is_ac_safety_timeout_enabled)
		{
			is_ac_safety_timeout_enabled = true;
			pr_info("Enable is_ac_safety_timeout_enabled\n");
			ret = pm_chg_failed_clear(chip, 1);
			if (ret)
				pr_err("Failed to write CHG_FAILED_CLEAR bit\n");
		}
...
static void unplug_check_worker(struct work_struct *work)
{
	...
	/*	Usb reverse boost issue happen on pmic8038.
	...
	if (reg_loop & VIN_ACTIVE_BIT) {
		...
		if (ibat > 0) {
			...
			while (count++ < param_vin_disable_counter && usb_chg_plugged_in == 1) {
				...
				if(!usb_chg_plugged_in)
					pr_info("cable out by unplug_check_worker() , count:%d\n", count);
...
static irqreturn_t chg_gone_irq_handler(int irq, void *data)
{
	...
	pr_info("chg_gone=%d, usb_valid = %d, fsm=%d\n",
								chg_gone, usb_chg_plugged_in, pm_chg_get_fsm_state(data));
	schedule_work(&chip->unplug_ovp_fet_open_work);
