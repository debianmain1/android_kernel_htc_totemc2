...
#if 0/*Mark, because we do not use fcc learning.*/
					...
					int batt_temp, int chargecycles, int verbol)
{
	...
	if (verbol) {
		pr_info("rbatt=%d,V_unusable_uv=%d,pc_unusable=%d,"
				"cal_pc=%d,scalefactor=%d,start_percent=%d,end_percent=%d,last_soc=%d\n",
				bms_dbg.rbatt, bms_dbg.voltage_unusable_uv, bms_dbg.pc_unusable,
				bms_dbg.cal_pc, bms_dbg.scalefactor, the_chip->start_percent,the_chip->end_percent,last_soc);
...
void pm8921_bms_charging_began(void)
{
	...
	pr_info("batt_temp = %d , start_percent = %d%%\n",batt_temp,the_chip->start_percent);
	bms_discharge_percent = 0;
