...
void htc_pm_monitor_work(struct work_struct *work)
{
	...
	if (htc_pm_monitor_wq == NULL) {
		printk(KERN_ERR "[K] hTc PM statistic is NILL.\n");
		return;
	}
	...
	htc_timer_stats_show(300, msm_htc_util_delay_time);/*Show timer events which greater than 300 every 10 sec*/
...
void htc_monitor_init(void)
{
	...
	if (htc_pm_monitor_wq) {
		printk(KERN_ERR "[K] htc_pm_monitor_wq is not NULL.\n");
